...
echo "account;user;policy_name;actions;resources"
for user in $(aws --profile ${prof} iam list-users | jq -r '.Users[].UserName'); do
	# Get list of attached policy ARNs for each rolei
	policy_names=($(aws --profile ${prof} iam list-user-policies --user-name ${user} | jq -r '.PolicyNames[]'))
	for policy_name in ${policy_names[@]}; do
		# Get default policy version
		policy_data=$(aws --profile ${prof} iam get-user-policy --user-name ${user} --policy-name ${policy_name})
		...
		while [ "${count}" -lt "${statement_length}" ]; do
			...
			if [ "${action_status}" == "True" ] && [ "${resource_status}" == "True" ]; then
				echo "${account};${user};${policy_name};${action};${resource}"
			fi
